function [x_flow, y_flow] = create_retro_flow_image(fig_handle, rf_path, im, ...
    ...
    flow_field_arrow_distance, token, running_tag)
...
im_inv = im_inv(x_shift +1:end, y_shift + 1:end, :);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
dx = ceil(abs(size(im_heat, 2) - size(r_flow, 2)) / 2);
dy = ceil(abs(size(im_heat, 1) - size(r_flow, 1)) / 2);

% im_out = im_heat(dy:dy+size(r_flow,1)-1, dx:dx+size(r_flow,2)-1, :);
imwrite(im_heat, fullfile(rf_path, 'images', ...
    ['flow_heatmaps_', token, '_', running_tag, '.tif']), ...
    'writemode', 'append');

% Overlay original image to flow heatmaps.
im = im(x_shift+1:end, y_shift+1:end);
im_inv_d(:,:,1) = imcomplement(im);
im_inv_d(:,:,2) = imcomplement(im);
im_inv_d(:,:,3) = imcomplement(im);
im_corr_cut_im = uint8(double(im_heat) .* im_inv_d);

imwrite(im_corr_cut_im, ...
    fullfile(rf_path, 'images', ...
    ['flow_heatmaps_overlay_', token, '_', running_tag, '.tif']), ...
    'writemode', 'append');
close(h);
